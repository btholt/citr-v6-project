...
// first run will create snapshot file
// future runs will check if the markup has changed
test("snapshot with no pets", () => {
  const tree = create(<Results pets={[]} />).toJSON();
  expect(tree).toMatchSnapshot();
});

// in our snapshot for Results.js, we end up rendering a ton of Pet components
// since we're testing Results, we dont care what Pet looks like
// if test in Results.js fails, we dont want it to be bc of something in Pet.js
// everything should have its own set of tests
// this is why we used shallow render, so it wont actually render Pet, just
// everything going in as props to Pet
